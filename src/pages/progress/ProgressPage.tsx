  ...
  return (
    <div className="space-y-6 pb-20 select-none">
      <div>
        ...
        <h1 className="text-2xl font-black text-zinc-100 tracking-tight">Progreso & Récords</h1>
        <p className="text-xs text-zinc-400 mt-0.5">Analiza tu evolución y superación de marcas por rutina y ejercicios</p>
      </div>

      {/* 1. SECTOR SUPERIOR: EXPLORADOR POR RUTINA Y DÍAS */}
      <Card className="p-4 sm:p-5 space-y-4 border-[#27272A] bg-[#18181B] shadow-sm">
        <div className="flex items-center justify-between border-b border-[#27272A] pb-3">
          <div className="flex items-center gap-2">
            <Layers className="w-4 h-4 text-gym-primary" />
          ...
          {routines.length > 1 && (
            <select
              ...
              className="px-2.5 py-1 text-xs font-bold bg-[#09090B] border border-[#27272A] rounded-xl text-zinc-100 focus:outline-none focus:border-gym-primary cursor-pointer"
            >
              {routines.map((r) => (
        ...
        {routines.length === 0 ? (
          <div className="p-4 rounded-xl bg-[#09090B] border border-[#27272A] text-center space-y-1">
            <p className="text-xs font-bold text-zinc-300">No tienes rutinas guardadas todavía.</p>
          ...
          <>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-base sm:text-lg font-bold text-zinc-100 flex items-center gap-1.5">
                {selectedRoutine?.isActive && <Star className="w-4 h-4 text-gym-primary fill-current" />}
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap flex items-center gap-1.5 flex-shrink-0 ${
                        isDaySelected
                          ...
                          : 'bg-[#09090B] text-zinc-400 hover:text-zinc-100 border border-[#27272A]'
                      }`}
                    >
            ...
            <div>
              <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider block mb-2">
                Ejercicios de este día (Toca uno para ver su evolución):
              </span>
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl border text-left transition-all flex flex-col justify-between ${
                          isSelected
                            ? 'border-gym-primary/80 bg-gym-primary/10 ring-1 ring-gym-primary/30'
                            : 'border-[#27272A] bg-[#09090B] hover:border-zinc-700'
                        }`}
                      >
                        <div>
                          <div className="flex items-center justify-between gap-1 mb-1">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-gym-primary">
                              {dayEx.exercise?.mainMuscleGroup || 'Fuerza'}
                            </span>
                            {isSelected && (
                              <span className="flex items-center gap-1 text-[10px] font-bold text-gym-primary bg-gym-primary/20 px-1.5 py-0.5 rounded-md">
                                <CheckCircle2 className="w-3 h-3" /> Viendo
                              </span>
                            )}
                          </div>
                          <h4 className="text-xs sm:text-sm font-bold text-zinc-100 leading-tight whitespace-normal break-words">
                            {exName}
                          </h4>
                        </div>

                        <div className="mt-2.5 pt-2 border-t border-[#27272A] flex items-center justify-between text-[11px]">
                          {exPR ? (
                            <span className="text-zinc-300 font-semibold flex items-center gap-1">
                              <span className="text-gym-primary">PR:</span>
                              <strong className="text-gym-primary font-bold">{exPR.value} kg × {exPR.reps} reps</strong>
                            </span>
                          ) : (
                          ...
                          <ChevronRight className="w-3.5 h-3.5 text-zinc-500 ml-1 flex-shrink-0" />
                        </div>
                      </button>
                    );
                  })}
      ...
      <Card className="p-4 border-[#27272A] bg-[#18181B] space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
            <Search className="w-3.5 h-3.5 text-gym-primary" />
            Ver otro ejercicio del catálogo
          </label>
          <span className="text-[10px] text-zinc-500">Catálogo general</span>
        </div>
        <select
          value={selectedExerciseId}
          onChange={(e) => setSelectedExerciseId(e.target.value)}
          className="w-full px-3.5 py-2.5 bg-[#09090B] border border-[#27272A] rounded-xl text-sm font-semibold text-zinc-100 focus:outline-none focus:border-gym-primary cursor-pointer"
        >
      ...
      <div className="space-y-4">
        ...
        <div className="bg-[#18181B] border border-[#27272A] rounded-2xl p-4 flex flex-wrap items-center justify-between gap-2 shadow-sm">
          <div>
            <span className="text-[10px] font-bold text-gym-primary uppercase tracking-widest block mb-0.5">
              Ejercicio Seleccionado
            </span>
            <h3 className="text-base sm:text-lg font-bold text-zinc-100 whitespace-normal break-words">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <div className="p-3.5 rounded-2xl bg-[#18181B] border border-[#27272A] flex flex-col items-center text-center shadow-sm">
            <Dumbbell className="w-5 h-5 text-gym-primary mb-1" />
            <span className="text-[10px] text-zinc-400 uppercase font-bold">Peso Máximo</span>
            <span className="text-lg font-black text-zinc-100">{maxWeightEver} kg</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-[#18181B] border border-[#27272A] flex flex-col items-center text-center shadow-sm">
            <Flame className="w-5 h-5 text-gym-primary mb-1" />
            <span className="text-[10px] text-zinc-400 uppercase font-bold">1RM Estimado</span>
            <span className="text-lg font-black text-gym-primary">{max1RMEver} kg</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-[#18181B] border border-[#27272A] flex flex-col items-center text-center shadow-sm">
            <Award className="w-5 h-5 text-gym-primary mb-1" />
            <span className="text-[10px] text-zinc-400 uppercase font-bold">Mayor Reps</span>
            <span className="text-lg font-black text-zinc-100">{maxRepsEver} reps</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-[#18181B] border border-[#27272A] flex flex-col items-center text-center shadow-sm">
            <TrendingUp className="w-5 h-5 text-gym-primary mb-1" />
            <span className="text-[10px] text-zinc-400 uppercase font-bold">Volumen Total</span>
            <span className="text-lg font-black text-zinc-100">{formatWeight(totalVolumeEver)}</span>
          </div>
        </div>

        {/* Gráfico 1: Evolución del Peso Máximo por Fecha */}
        <Card className="p-4 sm:p-5 space-y-3 border-[#27272A] bg-[#18181B]">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-zinc-100 tracking-tight flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4 text-gym-primary" />
              Evolución de Carga (kg)
            </h3>
            <span className="text-[10px] font-bold text-zinc-400">Por fecha</span>
          </div>
          ...
          {progressData.length === 0 ? (
            <div className="h-44 flex flex-col items-center justify-center text-zinc-500 text-xs">
              ...
              <span className="text-[11px] text-zinc-600 mt-1">Completa entrenamientos para trazar la curva.</span>
            </div>
          ) : (
            <div className="h-52 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={progressData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="formattedDate" stroke="#71717A" fontSize={10} tickLine={false} />
        ...
        <Card className="p-4 sm:p-5 space-y-3 border-[#27272A] bg-[#18181B]">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-zinc-400">Series × Reps × Peso</span>
          </div>
            ...
            <div className="h-44 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={progressData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  />
                  <Bar dataKey="totalVolume" fill="#84CC16" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <Card className="p-4 sm:p-5 space-y-3 border-[#27272A] bg-[#18181B] shadow-sm">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-zinc-100 tracking-tight flex items-center gap-1.5">
            <Trophy className="w-4 h-4 text-gym-primary" />
            RÉCORDS PERSONALES (PRs)
          </h3>
          <span className="text-[10px] font-bold text-zinc-400">Mejor marca por ejercicio</span>
        </div>
          ...
          <div className="space-y-2">
            {records.map((pr) => (
              <div
                ...
                className="p-3 rounded-xl bg-[#09090B] border border-[#27272A] hover:border-gym-primary/50 flex items-center justify-between transition-colors cursor-pointer"
                title="Toca para ver la evolución de este ejercicio"
              >
                <div>
                  <h4 className="text-xs font-bold text-zinc-100 whitespace-normal break-words">
                    {pr.exerciseName}
                  </h4>
                ...
                <div className="text-right flex-shrink-0 ml-2">
                  <span className="text-sm font-black text-gym-primary">
